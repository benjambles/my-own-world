...
export function createMigration(rl): void {
    rl.question(`Version type for this patch? [${validTypes.join(', ')}] default 'patch'`, function(
        type
    ): void {
        ...
        const currentVersion: string = migrations.length
            ? migrations[migrations.length].version
            : '1.0.0';
